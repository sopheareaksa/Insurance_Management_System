...
public class AuthController : Controller
{
    ...
    public async Task<IActionResult> RegisterSeller(RegisterSellerViewModel model)
    {
        ...
        try
        {
            ...
            TempData["SuccessMessage"] = $"Registration submitted and waiting for admin approval. Status: '{registeredSeller?.ApprovalStatus}'. Registration code: {registeredSeller?.RegNo}";
            return RedirectToAction(nameof(Login));
        }
        catch (SqlException ex)
        {
    ...
    public async Task<IActionResult> Login(string email, string password)
    {
        ...
        if (user != null)
        {
            try
            {
                passwordIsValid = BCrypt.Net.BCrypt.Verify(password, user.passwordHash);
            }
        ...
        if (string.Equals(user.role, "Agent", StringComparison.OrdinalIgnoreCase))
        {
            var company = await _context.Companies.FirstOrDefaultAsync(item => item.UserId == user.userId);
            if (company == null || !string.Equals(company.ApproveStatus, "Approved", StringComparison.OrdinalIgnoreCase))
            {
                TempData["ErrorMessage"] = company?.ApproveStatus == "Rejected"
                    ? "Your company registration was rejected. Please contact the administrator."
                    : "Your company registration is waiting for administrator approval.";
                return RedirectToAction(nameof(Login));
            }
        }
